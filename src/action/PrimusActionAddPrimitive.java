...
public class PrimusActionAddPrimitive extends AbstractContributionItemProvider implements IProvider {

	public static final String MENU_PRIMUSACTION1 = "primus_action_add_primitive"; //$NON-NLS-1$
	
	public PrimusActionAddPrimitive() {
	...
	public IMenuManager createMenuManager(String menuId, IWorkbenchPartDescriptor partDescriptor) {
		...
		MenuManager menuManager = new MenuManager("Add Primitive");
	...
	public class MenuBuilder implements IMenuListener {
		...
		public void buildMenu(IMenuManager manager){
			...
			for(i=0;i<11;i++) {
				...
				AddPrimitiveAction action = new AddPrimitiveAction(myWorkbenchPart,getWorkbenchPage(),package_, element, primitives[i]);
				action.init();
				manager.add(action);
			}
		}
