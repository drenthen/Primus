...
public class PrimusAction1 extends AbstractContributionItemProvider implements IProvider {

	public static final String MENU_PRIMUSACTION1 = "primus_action_1"; //$NON-NLS-1$
	
	public PrimusAction1() {
	...
	public IMenuManager createMenuManager(String menuId, IWorkbenchPartDescriptor partDescriptor) {
		...
		MenuManager menuManager = new MenuManager("Validate Primitives");
	...
	public class MenuBuilder implements IMenuListener {
		...
		public void buildMenu(IMenuManager manager){
			...
			primitives[6]="Shield";
			primitives[7]="Typing";
			primitives[8]="Virtual Connector";
			int i=0;
			for(i=0;i<9;i++) {
				ValidatePrimitiveAction action = new ValidatePrimitiveAction(getWorkbenchPage(), element, primitives[i]);
				action.init();
				manager.add(action);
			}
