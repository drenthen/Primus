...
import action.ValidatePrimitivesOfTypeAction.ValidatePrimitiveRequest;
//import org.eclipse.uml2.diagram.clazz.part.CustomMessages;
import org.eclipse.uml2.uml.Element;
import org.eclipse.uml2.uml.Stereotype;

public class ValidatePrimitivesOfTypeAction extends DiagramAction {
	private IWorkbenchWindow window;
	private static final String EMPTY_NAME = "emptyname";

	private Element myElement;
	private String myPrimitiveType;
	public ValidatePrimitivesOfTypeAction(IWorkbenchPage workbenchPage, Element element, String primitiveType) {
		super(workbenchPage);
		myElement = element;
	...
	protected Command getCommand() {
		...
		request.setDomain(editingDomain);
	...
	private class PrimitiveCommand extends EditElementCommand {
		...
		protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
			...
			OCLCheckAll check = new OCLCheckAll(primitiveType, element,request.getDomain());
			String message="";
			boolean success = check.validatePrimitive(request.getPrimitiveType());
			if(success) {
				message=check.resultString +"\n"+check.resultStringNear;
			} else {
				message="Query not successful\nTry Grouping OCL check first to initialize";
			}
